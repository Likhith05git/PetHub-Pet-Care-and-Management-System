		...
		try {
			if(req.getParameter("register")!=null) {
				...
				if(password.equals(confirm)) {
					...
					if(status.equals("Phone already registered")) {
						req.setAttribute("status", "Phone already registered");
						RequestDispatcher rd1=req.getRequestDispatcher("Registration.jsp");
						rd1.forward(req, resp);
					}
					else if(status.equals("Email already registered")) {
						req.setAttribute("status", "Email already registered");
						RequestDispatcher rd1=req.getRequestDispatcher("Registration.jsp");
						rd1.forward(req, resp);
					}
				...
				if(status.equals("success")) {
					...
					if(session.getAttribute("uname")!=null && session.getAttribute("id").equals("6")) {
				...
				if(status.equals("success")) {
	               ...
	               if (pw.equals(cp)) {
	                   ...
	                   if (status.equals("success")) {
	                       ...
	                       req.setAttribute("status", "email not found Create Account First");
	                       RequestDispatcher rd = req.getRequestDispatcher("ForgotReset.jsp");
	                       rd.forward(req, resp);
	                   }
	               } else {
